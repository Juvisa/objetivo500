...
window.AudioSynth = (() => {
  ...
  // ── guardianAmbient: ruido de fondo suave (reemplaza OGG con CORS) ──
  // Noise blanco filtrado + reverb sintético = atmósfera futurista
  let _ambientNodes = null;

  function synthGuardianAmbient(ctx, out) {
    try {
      if (_ambientNodes) return; // Ya activo

      const bufSec = 3;
      const rate   = ctx.sampleRate;
      const buf    = ctx.createBuffer(1, rate * bufSec, rate);
      ...
      _ambientNodes = { src, gain };
    } catch (e) { console.warn('[AudioSynth] guardianAmbient:', e); }
  }

  function stopGuardianAmbient(ctx) {
    try {
      if (!_ambientNodes) return;
      _ambientNodes.gain.gain.linearRampToValueAtTime(0, ctx.currentTime + 1);
      setTimeout(() => {
        try { _ambientNodes?.src.stop(); } catch (_) {}
        _ambientNodes = null;
      }, 1100);
    } catch (e) {}
  }

  return { synthSuccess, synthError, synthClick, synthPowerUp, synthLevelUp, synthStreak, synthDullThud, synthScoreTick, synthScoreFinale, synthGuardianAmbient, stopGuardianAmbient };
})();
