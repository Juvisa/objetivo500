...
window.Certificates = (() => {
  ...
  function _downloadById(certId, studentName) {
    const cert = _cache.find(c => c.id === certId);
    if (cert) download(cert, studentName);
  }

  function _setCache(certs) { _cache = certs; }
  ...
  return { getAll, evaluate, render, download, cardHTML, showNewCertToast, _downloadById, _setCache };
})();
