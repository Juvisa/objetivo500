...
window.Battle = (() => {
  ...
  async function create(challengerId, subject = null) {
    try {
      ...
      if (error) { console.error('[Battle.create] RPC error:', error); return { error }; }
      if (!battleId) return { error: { message: 'RPC devolvió null' } };
      return { battleId };
    } catch (e) {
      console.error('[Battle.create] excepción:', e);
  ...
  async function _doJoin(studentId) {
  ...
  return { create, join, getDetail, submitAnswer, getMyBattles, subscribeToChanges, unsubscribe, lobbyCardHTML, showJoinPrompt, _doJoin };
})();
