...
window.ExamCalendar = (() => {
  ...
  function widgetHTML(examData, studentId) {
    // Si no hay fecha personalizada, usar la próxima fecha oficial como default
    const isDefault = !examData;
    if (isDefault) {
      const defaultDateStr = _nextDefaultDate();
      if (!defaultDateStr) return '';   // Sin fechas futuras configuradas
      examData = { exam_date: defaultDateStr, exam_period: null, _isDefault: true };
    }
    ...
    return `
      <div class="ec-widget" style="border-color:${intensity.color};background:${intensity.bg}">
        ...
        <div class="ec-body" style="flex:1">
          ...
          <p class="ec-sub">${_formatDate(examData.exam_date)}${examData._isDefault ? ' · <a href="#" style="color:var(--text-muted);font-size:.75rem" onclick="ExamCalendar.openDatePicker(\''+studentId+'\');return false">¿Es tu fecha?</a>' : ''}</p>
